...
const ApiCoverPost = async function (req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'GET') {
    res.statusCode = 400
    res.end()
    return
  ...
  try {
    ...
    if (!post.coverPostImage) {
      res.statusCode = 404
      res.end()
      return
    }

    const { rawBody: image, headers: headers } = await got(post.coverPostImage)
